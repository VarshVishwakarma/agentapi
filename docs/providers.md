...
| ------------ | ---------------------------------------------------------------- | ------------------ |
| `openai`     | Optimized for general-purpose tasks, reasoning, and tool calling | `gpt-4o-mini`      |
| `gemini`     | Google's Gemini models with native function calling support      | `gemini-2.5-flash` |
| `openrouter` | Access multiple OpenAI-compatible models through OpenRouter      | `gpt-4o-mini`      |
...
If no provider is specified, AgentAPI uses the `DEFAULT_PROVIDER` environment variable automatically.
...
You can also explicitly override the model used by a provider:
...
Custom providers should inherit from `BaseProvider` and implement the required methods.
...
async def generate_response(
    ...
    Args:
        prompt (str):
            The user input or primary query.
